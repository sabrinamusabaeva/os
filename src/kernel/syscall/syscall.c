...
#include <SabrinaOS/kernel/syscall/console.h>
#include <SabrinaOS/kernel/syscall/file_handler.h>
#include <SabrinaOS/kernel/syscall/graphics.h>
#include <SabrinaOS/kernel/syscall/keyboard.h>
#include <SabrinaOS/kernel/syscall/syscall.h>
...
void interrupt_register_0x32_syscall() {
    ...
    populate_idt_entry_32bit(
        IDT_SYSCALL, (unsigned int)interrupt_handler_0x32_syscall_handler, 0,
        1);
...
int syscall_selector(int id, int arg0, int arg1, int arg2, int arg3,
                     int user_ds) {
    return ((int (*)(int, int, int, int, int))(SYSCALL_TABLE[id]))(
        arg0, arg1, arg2, arg3, user_ds);
}
