...
#include <SabrinaOS/memmgr/layout.h>
...
void process_scheduler_init() {
    ...
    populate_gdt_table(&gdtr, gdt_table, GDT_TABLE_SIZE,
                       MEMORY_KERNEL_LOCATION);
    load_gdt_table(&gdtr);
}

// return new user_sp
static int create_infant_process_argv_stack(int user_ds, int user_sp, int argc,
                                            char *argv[]) {
    ...
    syscall_strncpy_kernel_to_user(user_ds, __us_argv_list, ks_to_us_argv,
                                   sizeof(ks_to_us_argv));

    user_sp = user_sp - sizeof(argc);
    char *__us_argv = user_sp;
    syscall_strncpy_kernel_to_user(user_ds, __us_argv, &__us_argv_list,
                                   sizeof(__us_argv_list));
...
int process_create(unsigned int ppid, int argc, char *argv[]) {
    ...
    populate_gdt_entry(&gdt_table[idt_cs_entry], memory_location,
                       memory_size - 1, 0x9a,
                       GDT_ENTRY_FLAG_32_BIT_SELECTOR // 32-bit protected mode
    );
    // Application Data Segment Selector
    populate_gdt_entry(&gdt_table[idt_ds_entry], memory_location,
                       memory_size - 1, 0x92,
                       GDT_ENTRY_FLAG_32_BIT_SELECTOR // 32-bit protected mode
    );
    ...
    if (argc > 0) {
        ...
        process->sp = create_infant_process_argv_stack(
            process->ss, STACKINIT_APP, argc, argv);
        process->sp =
            create_infant_process_irq0_stack(process->ss, process->sp);
    }
    return pid;
}
...
int process_fork_mark_ready(int pid) {
    ...
    if (process->flagirq0_fork_ready > 0)
        return -3;                    // denied; fork already requested.
...
int process_fork_check_ready(int pid) {
    ...
    if (process == NULL)
        return -2;
    if (process->flagirq0_fork_ready == -1)
        return -3; // fork request: failed
    // bad state from user side
    if (process->flagirq0_fork_ready == 1)
        return -4; // fork request: still waiting
    return process
        ->flagirq0_fork_newchild; // fork request: success and return new pid
}
...
int process_fork(unsigned int ppid) {
    ...
    size_t size = min(memmgr_app_size(ppid), memmgr_app_size(npid));
    ...
    kernel_memncpy_absolute(dst_ds, 0, src_ds, 0, size);
    return npid;
}
