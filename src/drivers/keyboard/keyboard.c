...
void __stack_chk_fail(void) { PANIC(0, "Kernel stack overflow!!!"); }
...
void keyboard_wait() { sleep_mini(100000); }

const int WAIT_FOR_STATUS_TIMEOUT = 1000000;
const int WAIT_FOR_STATUS_KEYSCANCODE_TIMEOUT = 10;

int wait_for_status_flag_timeout_low(unsigned char flag,
                                     unsigned char check_true, int timeout,
                                     int canpanic) {
    while (timeout > 0) {
        if ((((port_read(DRIVERS_KEYBOARD_PORT_STATUS)) & flag) != 0) ^
            check_true) {
            // waiting
        } else {
            // poll done
...
int wait_for_status_flag_timeout(unsigned char flag, unsigned char check_true,
                                 int timeout) {
...
int wait_for_status_flag_nopanic(unsigned char flag, unsigned char check_true) {
    return wait_for_status_flag_timeout_low(flag, check_true,
                                            WAIT_FOR_STATUS_TIMEOUT, 0);
}
int wait_for_status_flag_nopanic_timeout(unsigned char flag,
                                         unsigned char check_true,
                                         int timeout) {
...
unsigned char ps2_controller_send_command_with_data(unsigned char cmd,
                                                    unsigned char data,
                                                    int want_reply) {
    unsigned char out;
    port_write(DRIVERS_KEYBOARD_PORT_COMMAND, cmd);
...
unsigned char write_to_ps2_first_port(unsigned char byte, int want_reply) {
    unsigned char out;
    wait_for_status_flag_timeout(STATUS_INPUT_BUFFER, 0,
    ...
    wait_for_status_flag_timeout(STATUS_OUTPUT_BUFFER, 1,
                                 WAIT_FOR_STATUS_TIMEOUT);
...
int keyboard_scanner_step() {
    ...
    if (qs < qc) {
        int got_response = wait_for_status_flag_nopanic_timeout(
            STATUS_OUTPUT_BUFFER, 1, WAIT_FOR_STATUS_KEYSCANCODE_TIMEOUT);
        if (!got_response)
            return 0;
